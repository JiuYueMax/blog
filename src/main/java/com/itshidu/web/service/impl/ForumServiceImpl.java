...
public class ForumServiceImpl implements ForumService {
	...
	public void findForumArticles(String forumCode, String sortType, int page, ModelAndView mv) {
		...
		Page<Article> data = _articleDao.findByForumCode(forumCode, pageable);
		mv.addObject("pageData", data);// 主要目的: 将分页信息传到页面,虽然也有文章信息但我不用
		...
		for (int i = 0; i < data.getContent().size(); i++) {
			...
			if(count == null) {
				count = _commentDao.countByArticleId(a.getId());//查询文章的评论数
			...
			if(favorCount==null) {
				...
				EhcacheUtil.put("mytest", key2, count, 10,10);
			}
