...
<link rel='stylesheet' media='all' href="/css/myPagination.css"/>
<script>

$(function() {
	new Page({
        id: 'pagebox',
        pageTotal: ${pageData.totalPages}, //必填,总页数
        pageAmount: ${pageData.numberOfElements},  //每页多少条
        dataTotal: ${pageData.totalElements}, //总共多少条数据
        curPage:${pageData.number+1}, //初始页码,不填默认为1
        ...
        getPage: function (page) {
            //获取当前页数
           console.log(page);
           window.location.href='?page='+page+'&ord=${RequestParameters.ord?default('')}';
        }
    })
});
</script>
