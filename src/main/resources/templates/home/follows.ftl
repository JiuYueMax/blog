...
<div class="shadow-box">
	...
	<#list voList as vo>
			<div class="stream-item" id="loop-${vo.id}">
				<div class="blog-rank">
					<div class="user" title="${vo.nickname}">
						<a href="/ta/${vo.id}">
    <img class="img-circle" src="${vo.avatar}"/>
						</a>
					</div>
				</div>
				<div class="summary">
					<h2 class="title">${vo.nickname}</h2>
					<div class="foot-block clearfix">
						<div class="author">
							<span>文章数 ${vo.articleCount}</span>
							<span>评论数 ${vo.commentCount}</span>
						</div>
						<div class="pull-right hidden-xs">
							<a class="btn btn-success btn-xs" href="javascript:void(0);" data-id="${vo.id}" rel="unfollow" style="color: #fff;">取消关注</a>
						</div>
					</div>
				</div>
			</div>
